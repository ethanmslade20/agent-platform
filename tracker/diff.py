...
def assign_loss_months(all_clients: pd.DataFrame) -> pd.DataFrame:
    ...
    verification-expired, left-book, or an undated cancellation) often has NO
    term_date. The month-over-month engine dates losses by term_date, so a
    ...
    Rule: a client is lost the moment a later snapshot shows them gone. We anchor
    the loss to `last_active` (the last month a snapshot showed them active) —
    "they were mine+active in an earlier photo, gone in a later one." A carrier
    portal that already gave a real cancel date keeps it. Plan switches
    ...
    left-book, plan-switch) so the final "who's gone" is known.
    """
